...
export default {
  ...
  data() {
    return {
      detailAray: [
        ...
        { title: "注册时间", value: "1658" },
        { title: "创建时间", value: "2018-12-07" },
        { title: "审核状态", value: "审核中" },
  ...
  mounted() {
    ...
    this.getDetail(this.currentRegNo);
    this.isOverdue();
  },
  watch: {
  ...
  methods: {
    ...
    getDetail(regNo) {
      const url = "/api/trademark/main/carefullyChosenTrademarkDetail";
      const params = {
        regNo: regNo,
      };
      this.$axios({
        ...
        .then(res => {
          if (res.data.success) {
            console.log(res);
            this.tradeMarkContent = res.data.data;
            this.detailAray[0].value = res.data.data.tmName;
            this.detailAray[1].value = res.data.data.regNo;
            this.detailAray[2].value =
              "第" + res.data.data.intCls + "类";
            this.detailAray[3].value =
              res.data.data.regDate;
            this.detailAray[4].value = res.data.data.createTime;
            if( res.data.data.verifyStatus===0){
                this.detailAray[5].value ='审核中'
            }
            if(res.data.data.verifyStatus===1){
                this.detailAray[5].value='已审核'
            }
            this.detailAray[6].value = res.data.data.agency;
            this.detailAray[7].value = "R型";
            this.detailAray[9].value =
              res.data.data.currentStatus;
          }
        })
        .catch(error => {
          console.log(error);
      ...
      if (accessToken) {
        if (this.isToken) {
          ...
          const params = {
            ...
            tmImg:  this.tradeMarkContent.tmImg,
